...
class ChurnPredictor:
    ...
    def __init__(self):
        ...
        for directory in [
            const.MODEL_PATH,
            const.EDA_IMAGES_PATH,
            const.RESULTS_IMAGES_PATH,
            os.path.dirname(
                const.LOG_PATH)]:
    ...
    def import_data(self, pth=const.DATA_PATH):
        ...
        try:
            ...
            logging.info(
                "Data import successful. Shape: %s", str(
                    self.df.shape))
            return True
    ...
    def perform_feature_engineering(self):
        ...
        try:
            ...
            missing_cols = [
                col for col in const.KEEP_COLS if col not in self.df.columns]
            if missing_cols:
    ...
    def train_models(self):
        ...
        try:
            ...
            if self.X_train is None or self.y_train is None:
                raise ValueError(
                    "Training data not available. Run feature engineering first.")

            # Random Forest
            rfc = RandomForestClassifier(random_state=const.RANDOM_STATE)
            cv_rfc = GridSearchCV(
                estimator=rfc, param_grid=const.RF_PARAM_GRID, cv=5)
            ...
            joblib.dump(
                ...
                os.path.join(
                    const.MODEL_PATH,
                    const.RFC_MODEL_NAME))
            joblib.dump(
                self.lr_model,
                os.path.join(
                    const.MODEL_PATH,
                    const.LOGISTIC_MODEL_NAME))
    ...
    def create_model_reports(self):
        ...
        try:
            ...
            plt.savefig(
                os.path.join(
                    const.RESULTS_IMAGES_PATH,
                    const.RFC_CLASSIFICATION_REPORT))
            plt.close()
            ...
            plt.savefig(
                os.path.join(
                    const.RESULTS_IMAGES_PATH,
                    const.LR_CLASSIFICATION_REPORT))
            plt.close()
