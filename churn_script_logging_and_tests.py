...
def test_import_data(churn_predictor):
    ...
    try:
        ...
        logging.error(
            "Testing import_data: Error occurred while testing data import")
        raise err
...
def test_encoder_helper(churn_predictor):
    ...
    try:
        ...
        for category in const.CATEGORICAL_COLUMNS:
            ...
            assert churn_predictor.df[new_column].dtype in [
                'float64', 'float32']
        ...
        logging.error(
            "Testing encoder_helper: Failed to encode categorical variables")
        raise err
...
def test_perform_feature_engineering(churn_predictor):
    ...
    try:
        ...
        assert len(churn_predictor.X_train) + \
            len(churn_predictor.X_test) == total_rows
        assert len(churn_predictor.y_train) + \
            len(churn_predictor.y_test) == total_rows

        logging.info("Testing perform_feature_engineering: SUCCESS")

    except AssertionError as err:
        logging.error(
            "Testing perform_feature_engineering: Failed to engineer features")
        raise err
